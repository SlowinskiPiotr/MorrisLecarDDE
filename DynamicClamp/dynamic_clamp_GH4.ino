...
const float gain_INPUT = 10.0;                    // number of millivolts sent out by amplifier for each millivolt of membrane potential
...
// These are for Jon Brown's breadboard for R1: 4700, R2: 470, R6: 100
//const float inputSlope = 5.6015/gain_INPUT;
//const float inputIntercept = -11910/gain_INPUT;
//const float outputSlope = 746.36/gain_OUTPUT;
//const float outputIntercept = 2385;

// These are for Jon Brown's breadboard for R1: 0, R2: Inf, R6: 4700
//const float inputSlope = 0.6649/gain_INPUT;
//const float inputIntercept = -1891.1146/gain_INPUT;
//const float outputSlope = 797.1688/gain_OUTPUT;
//const float outputIntercept = 2378.4453;

// These are for Kyle Wedgwood's breadboard for  R1: 4700, R2: 470, R6: 100
// const float inputSlope = 5.5455/gain_INPUT;
// const float inputIntercept = -11801/gain_INPUT;
// const float outputSlope = 749.02/gain_OUTPUT;
// const float outputIntercept = 2373;

//// These are for Kyle Wedgwood's breadboard for R1: 0, R2: Inf, R6: 4700
//const float inputSlope = 0.6688 / gain_INPUT;
//const float inputIntercept = -1971.293 / gain_INPUT;
//const float outputSlope = 814.6484 / gain_OUTPUT;
//const float outputIntercept = 2386.3584;

// These are for Kyle Wedgwood's new breadboard (on TW-E41-T1) for R6: 4.7k. R7: 10k, R8: 10k, R9: 4.7k (input gain 10, output gain 400)
const float inputSlope = 0.5035/gain_INPUT;
const float inputIntercept = -1110/gain_INPUT;
const float outputSlope = -589/gain_OUTPUT;
const float outputIntercept = 1928;

// Conductance parameters are sent by the host computer over the USB port
const int nPars = 3;              // number of adjustable parameters
float gLeak = 0.0;                // nS, shunting conductance
float kappa = 0.0;                // gain for delayed autapse
float tau = 10.0;                 // delay for tau

// Common global variables
float dt = 0.1;                   // msec, the time step -- on a Teensy/Arduino dt will be variable
elapsedMicros t0;                 // microsec, t0 is used to calculate dt on every iteration
elapsedMicros t1;                 // for debugging purposes

// Global variables for delay term
float t_hist[buffer_size] = {dt};                // buffer for delayed dt
float delay_sum = 0.0;
int ind_curr  = 0;
int ind_delay = 0;
...
void setup() {
  ...
  GenerateAutapseLUT();                       // generate autapse lookup table
}
...
void loop() {
  ...
  float injectionCurrent = 0.0;                                       // pA
    unsigned long sampling_delay = 50;

    while ( t0 < sampling_delay) {                                      // wait for sampling_period us
    }
  ...
  injectionCurrent += AutapseCurrent(v);
